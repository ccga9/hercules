...
class foroDAO extends DAO
{
    ...
    public function elimina($id)
    {
        ...
        $query = "SELECT id_r FROM foro WHERE id = '".$id."'";
        $ret_id = $this->consultarv2($query);
        $id_r = $ret_id[0]['id_r'];
        
        $query = "SELECT respuestas FROM foro WHERE id = '".$id_r."'";
        $resps = $this->consultarv2($query);
        $valor = $resps[0]['respuestas'];
        $valor--;
        $query = "UPDATE foro SET respuestas = '".$valor."' WHERE id = '".$id_r."'";
        $this->consultar($query);
